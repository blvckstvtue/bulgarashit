...
Config.storeCooldown = {
    ...
    amount = 1 -- how many of that time
}
...
Config.isHaker = {
    haker = math.random(100000, 999999),
    nothaker = math.random(10000000, 99999999),
} 
Config.MachRandomRegister = math.random(0, 1)

Config.register_loot = {
    item_needed = 'screwdriverset_blue', -- item name or nil
    ...
    reward = 'cash',  -- options: cash, bank , money(FOR ESX) or item name
    reward_amount = {2000, 2500}, -- {minimum, maximum}
    amount_type = 'info' -- options: info or item (info = metadata of 1 item) (item = amount of item) no need if you use cash or bank
}

Config.safe_loot = {
    item_needed = 'safehacking', -- item name or nil
    remove_item_success = false, --on success remove item true false or {0, number} to make it a chance it gets removed
    remove_item_fail = {0,1}, --on fail remove item true false or {0, number} to make it a chance it gets removed {0,1} == 50/50
    reward = 'cash', -- options: cash, bank , money(FOR ESX) or item name
    reward_amount = {3000, 5000}, -- {minimum, maximum}
...
Config.Dispatch = function(type, coords) -- type options safe or register
    if type == 'safe' then
        exports['sp-dispatch']:StoreRobbery(camId)
    elseif type == 'register' then
        exports['sp-dispatch']:StoreRobbery(camId)
    end
end
